...
const CreateShipperFirm = () => {
    const pTrackerContract = usePTrackerContract();

    const createFirm = async (values) => {
        console.log(values);
        try {
            const transaction = await pTrackerContract.createFirmAndOwner(values.firmOwner, values.firmName, values.firmLocation, false);
  ...
  return (
    <PageLayout page="Create Shipper Firm">
    <Formik
     ...
     >
       {({
         ...
         <form onSubmit={handleSubmit}>
            ...
            <Form.Label>Taşıyıcı Firma Metamask Cüzdan Adresi</Form.Label>
            <Form.Control 
                type="text" 
                name="firmOwner" 
           ...
           <Form.Group className="mt-3">
           <Form.Label>Taşıyıcı Firma Adı</Form.Label>
           <Form.Control 
                type="text" 
                name="firmName" 
                onChange={handleChange}
           ...
           <Form.Group className="mt-3">
           <Form.Label>Taşıyıcı Firma Adresi</Form.Label>
           <Form.Control 
                as="textarea" rows={3}
                name="firmLocation" 
                onChange={handleChange}
...
export default CreateShipperFirm;
